...
Native BNSH uses `64`-token logical sparse blocks by default. Explicit `128`-token blocks are still supported for compatibility, and both `m_block_dim` and `n_block_dim` must be multiples of `64`.

Current native BNSH limitations:

1. Forward only.
2. `dtype` must be `fp16` or `bf16`.
3. `head_dim` must be one of `32`, `64`, or `128`.
4. `m_block_dim` and `n_block_dim` must be multiples of `64`.
5. Dropout and `return_attn_probs=True` are not supported.
