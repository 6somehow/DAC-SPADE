...
#ifdef BLOCK_SPARSE_ATTN_ENABLE_TK
...
std::vector<torch::Tensor> tk_block_sparse_attention_forward(
    ...
    torch::Tensor q2k_block_sparse_index, torch::Tensor q2k_block_sparse_num) {
  TORCH_CHECK(false,
              "block_sparse_attn was built without ThunderKittens; "
              "set THUNDERKITTENS_ROOT to a ThunderKittens checkout and rebuild "
              "with BLOCK_SPARSE_ATTN_CUDA_ARCHS including 90a to enable fwd_tk.");
}
#endif
...
void set_bnsh_params_fprop(
    Flash_fwd_bnsh_params &params,
    // sizes
  ...
  params.k_ptr = k.data_ptr();
  params.v_ptr = v.data_ptr();
  // Native BNSH layout: [batch, heads, seqlen, dim].
...
constexpr int BNSH_BLOCK_SIZE = 64;

void run_mha_fwd_block_bnsh(Flash_fwd_bnsh_params &params,
                            cudaStream_t stream) {
  FP16_SWITCH(!params.is_bf16, [&] {
    BOOL_SWITCH(params.is_causal, Is_causal, [&] {
      switch (params.d) {
      case 32:
        run_mha_fwd_block_bnsh_<elem_type, 32, Is_causal>(params, stream);
        break;
      case 64:
        run_mha_fwd_block_bnsh_<elem_type, 64, Is_causal>(params, stream);
        break;
      case 128:
        run_mha_fwd_block_bnsh_<elem_type, 128, Is_causal>(params, stream);
        break;
      default:
        TORCH_CHECK(false,
                    "BNSH native forward only supports head dimensions 32, 64, and 128");
      }
    });
  });
}
...
int round_multiple_int(int x, int m) { return (x + m - 1) / m * m; }

void check_dense_cu_seqlens(const at::Tensor &cu_seqlens, int batch_size,
                            int seqlen, const char *name) {
  CHECK_DEVICE(cu_seqlens);
  CHECK_CONTIGUOUS(cu_seqlens);
  TORCH_CHECK(cu_seqlens.dtype() == torch::kInt32,
              std::string(name) + " must have dtype int32");
  CHECK_SHAPE(cu_seqlens, batch_size + 1);
  auto expected =
      (torch::arange(0, batch_size + 1,
                     torch::TensorOptions().dtype(torch::kInt32).device(
                         cu_seqlens.device())) *
       seqlen)
          .contiguous();
  TORCH_CHECK(torch::equal(cu_seqlens, expected),
              std::string(name) +
                  " must describe fully dense padded BNSH batches");
}

std::vector<at::Tensor> fwd_bnsh_impl(
    at::Tensor &q, const at::Tensor &k, const at::Tensor &v,
    const at::Tensor &head_mask_type,
    std::optional<at::Tensor> &streaming_info_,
    std::optional<at::Tensor> &row_blockmask_, int max_seqlen_q,
    const int max_seqlen_k, const float softmax_scale, bool is_causal,
    int window_size_left, int window_size_right, const int m_block_dim,
    const int n_block_dim, const bool exact_streaming) {
  at::cuda::CUDAGuard device_guard{q.device()};
  auto [cc_major, cc_minor] = get_compute_capability(get_current_device());
  bool is_sm8x_min = cc_major >= 8;
  TORCH_CHECK(is_sm8x_min,
              "BlockSparseAttention only supports Ampere GPUs or newer.");

  TORCH_CHECK(q.dim() == 4 && k.dim() == 4 && v.dim() == 4,
              "fwd_bnsh expects q, k, v to be 4D BNSH tensors");
  TORCH_CHECK(q.dtype() == torch::kFloat16 || q.dtype() == torch::kBFloat16,
              "BNSH native forward only supports fp16 and bf16 inputs");
  TORCH_CHECK(k.dtype() == q.dtype(), "query and key must have the same dtype");
  TORCH_CHECK(v.dtype() == q.dtype(),
              "query and value must have the same dtype");
  CHECK_DEVICE(q);
  CHECK_DEVICE(k);
  CHECK_DEVICE(v);
  TORCH_CHECK(q.stride(-1) == 1 && k.stride(-1) == 1 && v.stride(-1) == 1,
              "q, k, v must have contiguous last dimension");
  TORCH_CHECK(q.size(0) == k.size(0) && q.size(0) == v.size(0),
              "q, k, v batch sizes must match");
  TORCH_CHECK(k.size(1) == v.size(1), "k/v head counts must match");
  TORCH_CHECK(k.size(2) == v.size(2), "k/v sequence lengths must match");
  ...
  TORCH_CHECK(head_size == 32 || head_size == 64 || head_size == 128,
              "BNSH native forward only supports head dimensions 32, 64, and 128");
  TORCH_CHECK(k.size(3) == head_size && v.size(3) == head_size,
              "q, k, v head dimensions must match");
  TORCH_CHECK(num_heads % num_heads_k == 0,
              "Number of heads in key/value must divide number of heads in query");
  TORCH_CHECK(max_seqlen_q >= seqlen_q,
              "max_seqlen_q must be >= q.size(2)");
  TORCH_CHECK(max_seqlen_k >= seqlen_k,
              "max_seqlen_k must be >= k.size(2)");
  TORCH_CHECK(m_block_dim % BNSH_BLOCK_SIZE == 0,
              "m_block_dim must be a multiple of 64 (for example 64 or 128)");
  TORCH_CHECK(n_block_dim % BNSH_BLOCK_SIZE == 0,
              "n_block_dim must be a multiple of 64 (for example 64 or 128)");

  CHECK_DEVICE(head_mask_type);
  CHECK_CONTIGUOUS(head_mask_type);
  TORCH_CHECK(head_mask_type.dtype() == torch::kInt32,
              "head_mask_type must have dtype int32");
  ...
  if (has_streaming_info) {
    ...
    TORCH_CHECK(streaming_info.dtype() == torch::kInt32,
                "streaming_info must have dtype int32");
  ...
  if (has_blockmask) {
    ...
    TORCH_CHECK(row_blockmask.dtype() == torch::kInt32,
                "row_blockmask must have dtype int32");
  }

  const int max_mask_type = head_mask_type.max().item<int>();
  const int min_mask_type = head_mask_type.min().item<int>();
  TORCH_CHECK(!exact_streaming || (is_causal && has_streaming_info),
              "exact_streaming requires causal mode and streaming_info");
  TORCH_CHECK(min_mask_type >= 0 || has_streaming_info,
              "streaming_info is required when any head uses streaming");

  int num_blocksparse_heads = 0;
  if (max_mask_type > 0) {
    TORCH_CHECK(has_blockmask,
                "row_blockmask is required when any head uses block-sparse attention");
    num_blocksparse_heads = row_blockmask.size(1);
  } else if (has_blockmask) {
    num_blocksparse_heads = row_blockmask.size(1);
  }
  TORCH_CHECK(max_mask_type <= num_blocksparse_heads,
              "row_blockmask.shape[1] must cover all positive head_mask_type ids");

  const int seqlen_q_rounded = round_multiple_int(max_seqlen_q, m_block_dim);
  const int seqlen_k_rounded = round_multiple_int(max_seqlen_k, n_block_dim);
  if (has_blockmask) {
    CHECK_SHAPE(row_blockmask, batch_size, num_blocksparse_heads,
                seqlen_q_rounded / m_block_dim, seqlen_k_rounded / n_block_dim);
  }
  ...
  at::Tensor rng_state = torch::empty(
      {0}, torch::TensorOptions().dtype(torch::kInt64).device(q.device()));

  Flash_fwd_bnsh_params params;
  set_bnsh_params_fprop(params, batch_size, seqlen_q, seqlen_k, seqlen_q_rounded,
                        seqlen_k_rounded, num_heads, num_heads_k, head_size,
                        /*d_rounded=*/head_size, q, k, v, out,
                        /*cu_seqlens_q_d=*/nullptr, /*cu_seqlens_k_d=*/nullptr,
                        /*seqused_k=*/nullptr, /*p_d=*/nullptr,
                        softmax_lse.data_ptr(),
                        /*p_dropout=*/0.0f, softmax_scale, window_size_left,
  ...
  params.blockmask =
      has_blockmask ? static_cast<int *>(row_blockmask.data_ptr()) : nullptr;
  params.streaming_info =
      has_streaming_info ? static_cast<int *>(streaming_info.data_ptr())
                         : nullptr;
  ...
  if (seqlen_k > 0) {
    ...
    run_mha_fwd_block_bnsh(params, stream);
  } else {
    out.zero_();
...
std::vector<at::Tensor> mha_fwd_block_bnsh(
    at::Tensor &q, const at::Tensor &k, const at::Tensor &v,
    const at::Tensor &cu_seqlens_q, const at::Tensor &cu_seqlens_k,
    at::Tensor &row_blockmask, float softmax_scale, bool is_causal,
    int m_block_dim, int n_block_dim, bool return_softmax,
    std::optional<at::Generator> gen_) {
  (void)gen_;
  TORCH_CHECK(!return_softmax,
              "Native BNSH forward does not support return_softmax");
  const int batch_size = q.size(0);
  const int seqlen_q = q.size(2);
  const int seqlen_k = k.size(2);
  check_dense_cu_seqlens(cu_seqlens_q, batch_size, seqlen_q, "cu_seqlens_q");
  check_dense_cu_seqlens(cu_seqlens_k, batch_size, seqlen_k, "cu_seqlens_k");

  auto head_mask_type = torch::arange(
      1, q.size(1) + 1,
      torch::TensorOptions().dtype(torch::kInt32).device(q.device()));
  std::optional<at::Tensor> no_streaming = std::nullopt;
  std::optional<at::Tensor> blockmask = row_blockmask;
  return fwd_bnsh_impl(
      q, k, v, head_mask_type, no_streaming, blockmask, seqlen_q, seqlen_k,
      softmax_scale, is_causal, /*window_size_left=*/-1,
      /*window_size_right=*/is_causal ? 0 : -1, m_block_dim, n_block_dim,
      /*exact_streaming=*/false);
}

std::vector<at::Tensor>
fwd_bnsh(at::Tensor &q, const at::Tensor &k, const at::Tensor &v,
         const at::Tensor &head_mask_type,
         std::optional<at::Tensor> &streaming_info_,
         std::optional<at::Tensor> &row_blockmask_, int max_seqlen_q,
         const int max_seqlen_k, const float p_dropout,
         const float softmax_scale, bool is_causal, int window_size_left,
         int window_size_right, const int m_block_dim, const int n_block_dim,
         const bool exact_streaming, const bool return_softmax,
         std::optional<at::Generator> gen_) {
  (void)gen_;
  TORCH_CHECK(p_dropout == 0.0f,
              "Native BNSH forward does not support dropout");
  TORCH_CHECK(!return_softmax,
              "Native BNSH forward does not support return_softmax");
  return fwd_bnsh_impl(
      q, k, v, head_mask_type, streaming_info_, row_blockmask_, max_seqlen_q,
      max_seqlen_k, softmax_scale, is_causal, window_size_left,
      window_size_right, m_block_dim, n_block_dim, exact_streaming);
}
...
std::vector<at::Tensor> mha_varlen_fwd_block(
    ...
    std::optional<at::Tensor>
        &row_blockmask_, // (batch_size, num_blocksparse_heads, seqlen_m /
                         // m_block_dim, seqlen_n / n_block_dim)
  ...
  TORCH_CHECK(
      head_size <= 256,
      "BlockSparseAttention forward only supports head dimension at most 256");
  ...
  auto softmax_lse = torch::empty({batch_size, num_heads, max_seqlen_q},
                                  opts.dtype(at::kFloat));
  // Always return a valid tensor for `p` (used as S_dmask / attn probs on the
  // Python side). When return_softmax=false we return an empty tensor to avoid
  // undefined behavior.
  at::Tensor p = torch::empty({0}, opts);
  ...
  set_params_fprop(
      ...
      k, v, out, cu_seqlens_q.data_ptr(), cu_seqlens_k.data_ptr(), nullptr,
  ...
  // IMPORTANT (PyTorch custom_op): returns must not alias inputs or other
  // returns. Return a minimal, non-aliased set of outputs.
  return {out, softmax_lse, p, rng_state};
}
...
std::vector<at::Tensor> mha_varlen_bwd_block(
    ...
    std::optional<at::Tensor> &streaming_info_,
    std::optional<at::Tensor>
        &col_blockmask_, // (batch_size, num_blocksparse_heads, seqlen_n /
                         // n_block_dim, seqlen_m / m_block_dim)
    const int max_seqlen_q,
    const int max_seqlen_k, // max sequence length to choose the kernel
    const float p_dropout,  // probability to drop
    const float softmax_scale, const bool zero_tensors, const bool is_causal,
    int window_size_left, int window_size_right, const int m_block_dim,
    const int n_block_dim, const bool deterministic,
  ...
  TORCH_CHECK(head_mask_type.dtype() == torch::kInt32,
              "head_mask_type must have dtype int32");
  if (has_blockmask) {
    TORCH_CHECK(col_blockmask.dtype() == torch::kInt32,
                "col_blockmask must have dtype int32");
    TORCH_CHECK(m_block_dim % SPARSE_SIZE == 0,
                "m_block_dim must be a multiple of 128");
    TORCH_CHECK(n_block_dim % SPARSE_SIZE == 0,
                "n_block_dim must be a multiple of 128");
  ...
  TORCH_CHECK(
      head_size <= 256,
      "BlockSparseAttention backward only supports head dimension at most 256");
  TORCH_CHECK(
      num_heads % num_heads_k == 0,
      "Number of heads in key/value must divide number of heads in query");
  ...
  if (has_blockmask) {
    ...
    CHECK_SHAPE(col_blockmask, batch_size, num_blocksparse_heads,
                seqlen_k_rounded / n_block_dim, seqlen_q_rounded / m_block_dim);
  }
  if (has_streaming_info) {
    CHECK_SHAPE(streaming_info, num_heads * 2);
  }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("fwd_block", &FLASH_NAMESPACE::mha_varlen_fwd_block,
        "Forward pass, with blockmask");
  m.def("mha_fwd_block_bnsh", &FLASH_NAMESPACE::mha_fwd_block_bnsh,
        "Forward pass (BNSH), with blockmask");
  m.def("fwd_bnsh", &FLASH_NAMESPACE::fwd_bnsh,
        "Forward pass (BNSH), with blockmask");
  m.def("fwd_tk", &tk_block_sparse_attention_forward,
        "Forward pass (Hopper/ThunderKittens), with blockmask");
  m.def("bwd_block", &FLASH_NAMESPACE::mha_varlen_bwd_block,
        "Backward pass, with blockmask");
}
