...
def get_fwd_block_bnsh_template() -> str:
    return NAMESPACE_INCLUDE + """#include "flash_fwd_launch_template_bnsh.h"

namespace FLASH_NAMESPACE {{

template<>
void run_mha_fwd_block_bnsh_<{DTYPE}, {HEAD_DIM}, {IS_CAUSAL}>(Flash_fwd_bnsh_params &params, cudaStream_t stream) {{
    run_mha_fwd_block_bnsh_hdim{HEAD_DIM}<{DTYPE}, {IS_CAUSAL}>(params, stream);
}}
...
class Kernel:
    ...
    def template(self) -> str:
        template_funcs = {
            ...
            "fwd_block_bnsh": get_fwd_block_bnsh_template,
        }
    ...
    def filename(self) -> str:
        if self.direction == "fwd_block_bnsh":
            return (
                f"flash_{self.direction}_hdim{self.head_dim}_{self.dtype}_"
                f"{'causal_' if self.is_causal == 'true' else ''}sm{self.sm}.cu"
            )
...
def get_all_kernels() -> List[Kernel]:
    for direction in ["fwd_block", "bwd_block", "fwd_block_bnsh"]:
