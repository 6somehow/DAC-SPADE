 ...
 * https://github.com/Dao-AILab/flash-attention/blob/main/csrc/flash_attn/src/flash.h
 ******************************************************************************/
...
struct Flash_fwd_bnsh_params : public Qkv_params {
  // The O matrix (output).
  void *__restrict__ o_ptr;
  void *__restrict__ oaccum_ptr;
  ...
  // The pointer to the P matrix and softmax lse.
  void *__restrict__ p_ptr;
  void *__restrict__ softmax_lse_ptr;

  // Dimensions.
  int b, seqlen_q, seqlen_k, seqlen_knew, d, seqlen_q_rounded,
      seqlen_k_rounded, d_rounded, total_q;
  ...
  // Forward-compatible KV cache fields required by shared kernel helpers.
  void *__restrict__ knew_ptr;
  void *__restrict__ vnew_ptr;
  index_t knew_batch_stride;
  index_t vnew_batch_stride;
  ...
  // Dropout values (forward-compatible, even if currently disabled in BNSH).
  float p_dropout;
  uint8_t p_dropout_in_uint8_t;
  float rp_dropout;
  float scale_softmax_rp_dropout;
  ...
  // Optional bias fields required by shared kernel helpers.
  void *__restrict__ alibi_slopes_ptr;
  index_t alibi_slopes_batch_stride;
...
template <typename T, int Headdim, bool Is_causal>
void run_mha_bwd_block_(Flash_bwd_params &params, cudaStream_t stream);

template <typename T, int Headdim, bool Is_causal>
void run_mha_fwd_block_bnsh_(Flash_fwd_bnsh_params &params,
                             cudaStream_t stream);
