...
def _get_block_sparse_extension(required: bool = False):
    return load_extension(required=required)
...
def _check_qkv_dtype_device(q, k, v):
...
def _check_mask_inputs(base_blockmask, blocksparse_head_num):
    if base_blockmask is not None:
        assert base_blockmask.shape[1] == blocksparse_head_num


def _normalize_softmax_scale(softmax_scale, q):
    return q.shape[-1] ** (-0.5) if softmax_scale is None else softmax_scale


def _maybe_pad_last_dim(x, multiple=8):
    head_size_og = x.size(-1)
    if head_size_og % multiple != 0:
        x = torch.nn.functional.pad(x, [0, multiple - head_size_og % multiple])
    return x, head_size_og


def _to_row_blockmask(base_blockmask, is_causal):
    if base_blockmask is None:
        return None
    return convert_blockmask_row_reverse(base_blockmask, is_causal)


def _to_col_blockmask(base_blockmask, is_causal):
    if base_blockmask is None:
        return None
    return convert_blockmask_col_reverse(base_blockmask, is_causal)


def _check_exact_streaming_inputs(exact_streaming, streaming_info, is_causal):
    if exact_streaming:
        assert streaming_info is not None
        assert is_causal


def _save_ctx_for_backward(
    ctx,
    q,
    k,
    v,
...
def _block_sparse_attn_forward(
    ...
    out, softmax_lse, S_dmask, rng_state = ext.fwd_block(
        q, k, v,
...
def _block_sparse_attn_backward(
    ...
    ext = _get_block_sparse_extension(required=True)
    dq, dk, dv, softmax_d = ext.bwd_block(
        dout,
        q, k, v,
        out,
        softmax_lse,
...
class BlockSparseAttnFunc(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        q, _ = _maybe_pad_last_dim(q)
        k, _ = _maybe_pad_last_dim(k)
        v, _ = _maybe_pad_last_dim(v)
    ...
    def backward(ctx, dout, *args):
        ...
        dout_padded, _ = _maybe_pad_last_dim(dout)
        col_blockmask = _to_col_blockmask(base_blockmask, ctx.is_causal)
