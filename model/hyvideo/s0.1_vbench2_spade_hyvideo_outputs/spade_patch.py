...
def dync_policy_func(start_timestep: int, total_timestep: int,
    ...
    # start_factor = 0.25
    # end_factor = 0.05
    # base_block = (start_factor - (timestep - start_timestep) *
    #               (start_factor - end_factor) /
    #               (total_timestep - start_timestep)) * total_block
    inter_top_val = total_block * 0.10
    diag_width = 2
    intra_top_val = 0.99
    return int(inter_top_val), intra_top_val, int(diag_width)
...
def select_sparse_engine(backend='torch'):
    ...
    spatial_config2 = SparseHeadConfig(
        hidden_dim=hidden_dim,
        seqlen3d=seqlen3d,
        block_size_q=(1, 16, 4),
        block_size_kv=(1, 16, 4),
        fixed_diag_width=70,
        fixed_sink_width=10,
        context_length=context_length,
        inter_select_mode='topk',
    ...
    temporal_config2 = SparseHeadConfig(
        hidden_dim=hidden_dim,
        seqlen3d=seqlen3d,
        block_size_q=(32, 1, 2),
        block_size_kv=(32, 1, 2),
        fixed_diag_width=70,
        fixed_sink_width=10,
        context_length=context_length,
        inter_select_mode='topk',
    ...
    spatial_config_id2 = exec_table.register_config(spatial_config2)
    ...
    temporal_config_id2 = exec_table.register_config(temporal_config2)
