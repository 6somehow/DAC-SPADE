...
class Inference(object):
    ...
    @staticmethod
    def _resolve_dit_weight_path(dit_weight, pretrained_model_path):
        if dit_weight is None:
            return None

        dit_weight = Path(dit_weight)
        if dit_weight.exists() or dit_weight.is_absolute():
            return dit_weight

        candidates = [dit_weight]
        repo_hyvideo_root = Path(__file__).resolve().parents[1]
        candidates.append(repo_hyvideo_root / dit_weight)

        if pretrained_model_path is not None:
            pretrained_model_path = Path(pretrained_model_path)
            candidates.append(pretrained_model_path / dit_weight)
            if dit_weight.parts and dit_weight.parts[0] == "ckpts":
                trimmed = Path(*dit_weight.parts[1:])
                candidates.append(pretrained_model_path / trimmed)
                if len(dit_weight.parts) > 1 and pretrained_model_path.name == dit_weight.parts[1]:
                    candidates.append(pretrained_model_path / Path(*dit_weight.parts[2:]))
    ...
    def load_state_dict(args, model, pretrained_model_path):
        ...
        dit_weight = Inference._resolve_dit_weight_path(args.dit_weight, pretrained_model_path)
