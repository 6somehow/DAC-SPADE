...
def attention(
    ...
    if mode == "torch":
        ...
        if cu_seqlens_q is None:
            ...
            attn1 = F.scaled_dot_product_attention(q[:, :, :cu_seqlens_q[1]],
                                                   k[:, :, :cu_seqlens_kv[1]],
                                                   v[:, :, :cu_seqlens_kv[1]],
                                                   attn_mask=attn_mask,
                                                   dropout_p=drop_rate,
                                                   is_causal=causal)
            attn2 = F.scaled_dot_product_attention(q[:, :, cu_seqlens_q[1]:],
                                                   k[:, :, cu_seqlens_kv[1]:],
                                                   v[:, :, cu_seqlens_kv[1]:],
                                                   attn_mask=None,
                                                   dropout_p=drop_rate,
...
def parallel_attention(hybrid_seq_parallel_attn, q, k, v, img_q_len,
                       img_kv_len, cu_seqlens_q, cu_seqlens_kv):
