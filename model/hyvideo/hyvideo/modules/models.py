...
from .spade_patch import spade_engine, is_spade_engine
...
class MMDoubleStreamBlock(nn.Module):
    ...
    def forward(
        ...
        if not self.hybrid_seq_parallel_attn:
            ...
            if is_spade_engine:
                attn = spade_engine(q, k, v, cu_seqlens_q, cu_seqlens_kv,
                                    max_seqlen_q,
                                    max_seqlen_kv).flatten(-2, -1)
            else:
                # print("Double Stream:", q.size(), k.size(), v.size(), cu_seqlens_q,
                #       cu_seqlens_kv)
                attn = attention(
                    q,
                    k,
            ...
            attn = parallel_attention(self.hybrid_seq_parallel_attn,
                                      q,
                                      k,
                                      v,
    ...
    def forward(
        ...
        if not self.hybrid_seq_parallel_attn:
            if is_spade_engine:
                ...
                # print("Single Stream:", q.size(), k.size(), v.size(),
...
class HYVideoDiffusionTransformer(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        text_states_2: Optional[
            torch.Tensor] = None,  # Text embedding for modulation.
        freqs_cos: Optional[torch.Tensor] = None,
        freqs_sin: Optional[torch.Tensor] = None,
        guidance: torch.
        Tensor = None,  # Guidance for modulation, should be cfg_scale x 1000.
