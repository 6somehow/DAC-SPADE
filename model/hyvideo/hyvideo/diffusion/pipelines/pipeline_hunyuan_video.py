...
class HunyuanVideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        device = torch.device(
            f"cuda:{dist.get_rank()}") if dist.is_initialized(
        ...
        autocast_enabled = (
            target_dtype != torch.float32) and not self.args.disable_autocast
        ...
        from ...modules.spade_patch import spade_engine, is_spade_engine
        # if is_progress_bar:
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if is_spade_engine:
                    assert spade_engine.now_timestep == i, f"spade_engine.now_timestep != i, {spade_engine.now_timestep} != {i}"
                if self.interrupt:
                    continue
        ...
        from ...modules.spade_patch import is_spade_engine, spade_engine
        if is_spade_engine and spade_engine.record_sparse_rate:
            torch.save(spade_engine.record_sim, 'record_sim.pt')
            torch.save(spade_engine.sparse_pattern, 'record_attn.pt')
            torch.save(spade_engine.sparse_rate, 'sparse_rate.pt')
