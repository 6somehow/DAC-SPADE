...
class WanI2V:
    ...
    def generate(self,
        ...
        print((F - 1) // self.vae_stride[0] + 1, lat_h, lat_w)
        ...
        with (
            ...
            index_boundary = torch.where(timesteps >= boundary)[0][-1].item()
            print(
                f"high noise model [0,{index_boundary}], low noise model [{index_boundary+1},{timesteps.size(0)}]"
            )
            # sample videos
            latent = noise
