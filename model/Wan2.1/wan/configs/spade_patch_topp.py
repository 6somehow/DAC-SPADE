...
def estimate_func_mean(block_q, block_k):
    q_mean = block_q['Mean']
    attn_weights_mean = torch.matmul(q_mean, block_k['Mean'].transpose(-1, -2))
...
def select_sparse_engine(backend='torch'):
    ...
    spatial_block_size_q0 = (1, 16, 4)
    spatial_block_size_kv0 = (1, 16, 4)
    ...
    inter_top_val[start_timestep:, 0, start_layer:, :] = 0.9999
    intra_top_val[start_timestep:, 0, start_layer:, :] = 10

    inter_top_val[start_timestep:, 1, start_layer:, :] = 0.99
